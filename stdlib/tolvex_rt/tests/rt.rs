...
fn session_windows_splits_on_inactivity_gap() {
    ...
    assert_eq!(
        ...
        vec![&events[0..2], &events[2..4]]
    );
}

#[test]
fn session_windows_splits_on_out_of_order_timestamps() {
    // "c" arrives with an earlier timestamp than "b" (reordered delivery);
    // it must not be silently merged into the same session via a clamped
    // zero-length gap.
    let events = vec![(1000u64, "a"), (1050, "b"), (500, "c")];
    assert_eq!(
        session_windows(&events, 10_000),
        vec![&events[0..2], &events[2..3]]
    );
}
...
fn alert_dispatcher_notifies_on_matching_condition() {
    ...
    dispatcher.check("hr", 100.0); // boundary-exact reading must also alert
    dispatcher.check("bp", 200.0);

    assert_eq!(dispatcher.sink().alerts.len(), 2);
    assert_eq!(dispatcher.sink().alerts[0].value, 150.0);
    assert_eq!(dispatcher.sink().alerts[1].value, 100.0);
}
...
fn mqtt_adapter_publish_stub_returns_err() {
    ...
    let mut adapter = MqttAdapter::new("mqtt://localhost:1883", "vitals/hr");
...
fn amqp_adapter_publish_stub_returns_err() {
    ...
    let mut adapter = AmqpAdapter::new("amqp://localhost", "vitals", "hr.reading");
    let reading = DeviceReading::new("dev1", "hr", 72.0, 1000);
    assert!(adapter.connect().is_err());
    assert!(adapter.publish(&reading).is_err());
}
